            ...
            if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
            {
                using (var package = new ExcelPackage(file.InputStream))
                {
                    ...
                    for (int rowIterator = 4; rowIterator <= noOfRow; rowIterator++)
                    {
                        ...
                        if(checkDate && payInfo.VehicleID.HasValue && payInfo.DriverID.HasValue)
                        {
                            var firstDayOfMonth = new DateTime(payInfo.PayDate.Year, payInfo.PayDate.Month, 1);
                            var lastDayOfMonth = firstDayOfMonth.AddMonths(1).AddDays(-1);

                            var check = driverPays.Where(x => x.VehicleID == payInfo.VehicleID && x.DriverID == payInfo.DriverID && x.IsRemove != true 
                        ...
                        var content = CastToProperty<string>(workSheet, rowIterator, 2, "Nội dung sửa chữa", false);
