...
namespace WEB.Areas.Admin.Controllers
{
    ...
    public class ManageDayTicketController : Controller
    {
        ...
        {
            var model = db.ManageTickets.Where(x => x.IsRemove != true && x.TicketType == (int)TypeOfTicket.DayTicket).ToList();
        ...
        public ActionResult DayDetail(string dataString)
        {
            if (dataString != null)
            {
        ...
        public ActionResult DayTickets_Read([DataSourceRequest] DataSourceRequest request, string datetime)
        {
            ...
            var repairVehicleQuery = from a in db.ManageTickets.Where(x => DbFunctions.TruncateTime(firstDay) <= DbFunctions.TruncateTime(x.TicketDate) && DbFunctions.TruncateTime(x.TicketDate) <= DbFunctions.TruncateTime(lastDay) && x.TicketType == (int)TypeOfTicket.DayTicket && x.IsRemove != true)
        ...
        public ActionResult Add([Bind(Exclude = "")] ManageTicket model, DateTime? date)
        {
            if (ModelState.IsValid)
            {
                if (date == null)
                {
                    ModelState.AddModelError("", AccountResources.RequiredDate);
                    return View(model);
                }
                ...
                var check = db.ManageTickets.Where(x => x.VehicleID == model.VehicleID && x.DriverID == model.DriverID && x.IsRemove != true && x.TicketType == (int)TypeOfTicket.DayTicket
                ...
                {
                    model.TicketType = (int)TypeOfTicket.DayTicket;
        ...
        public ActionResult Edit([Bind(Include = "")] ManageTicket model, DateTime? date)
        {
            if (ModelState.IsValid)
            {
                ...
                var check = db.ManageTickets.Where(x => x.VehicleID == model.VehicleID && x.DriverID == model.DriverID && x.IsRemove != true && x.TicketType == (int)TypeOfTicket.DayTicket && x.ID != model.ID
                && DbFunctions.TruncateTime(firstDayOfMonth) <= DbFunctions.TruncateTime(x.TicketDate) && DbFunctions.TruncateTime(x.TicketDate) <= DbFunctions.TruncateTime(lastDayOfMonth)).Any();
                if (check)
                {
                    ModelState.AddModelError("", "Đã tồn tại bản ghi trùng tên lái xe và biển kiểm soát trên hệ thống!");
        ...
        public ActionResult ExportExcelFromIndex(string data)
        {
            ...
            foreach (var item in lstDate)
            {
                ...
                var manageTicketQuery = (from a in db.ManageTickets.Where(x => DbFunctions.TruncateTime(firstDayOfMonth) <= DbFunctions.TruncateTime(x.TicketDate) && DbFunctions.TruncateTime(x.TicketDate) <= DbFunctions.TruncateTime(lastDayOfMonth) && x.TicketType == (int)TypeOfTicket.DayTicket && x.IsRemove != true)
                                         join b in db.Vehicle on a.VehicleID equals b.ID into group1
                                         from b in group1.DefaultIfEmpty()
                                         join d in db.Vehicle on a.DriverID equals d.ID into group3
                                         from d in group3.DefaultIfEmpty()
            ...
            return File(fileStream, "application/ms-excel", "QL_Ve_Ngay.xlsx");
        }
        ...
        public ActionResult ExportExcel(string dataString)
        {
            ...
            var result = DownloadManageTicket(listData);
            var fileStream = new MemoryStream(result);
            return File(fileStream, "application/ms-excel", "QL_Ve_Ngay.xlsx");
        }
        ...
        public ActionResult UploadExcel(string submit)
        {
            if (submit == "Upload")
            {
                ...
                if (check == 1)
                {
                    var result = new UploadManageTicketFromExcel().UploadProducts(file, Session["UploadRepairProgress"],(int)TypeOfTicket.DayTicket);


                    if (result == null)
                    {
                        ViewBag.check = "Upload quản lý vé ngày thành công!";
            ...
            else
            {
                ...
                var fileStream = new MemoryStream(result);
                return File(fileStream, "application/ms-excel", "QLVeNgay_Template.xlsx");
            }
        }
        ...
        public ActionResult DeleteFile(string dataString)
        {
            ...
            return File(dateGet, "application/ms-excel", "QLVeNgay_UploadResult.xlsx");
        ...
        public ActionResult Deletes(string stringSubmit)
        {
            ...
            foreach (var date in lstDate)
            {
                var firstDayOfMonth = new DateTime(date.Year, date.Month, 1);
                var lastDayOfMonth = firstDayOfMonth.AddMonths(1).AddDays(-1);
                var listTicket = db.ManageTickets.Where(x => DbFunctions.TruncateTime(firstDayOfMonth) <= DbFunctions.TruncateTime(x.TicketDate) && DbFunctions.TruncateTime(x.TicketDate) <= DbFunctions.TruncateTime(lastDayOfMonth) && x.IsRemove != true && x.TicketType == (int)TypeOfTicket.DayTicket).ToList();
        ...
        public ActionResult DeletesFromDetail(string dataStringDelete, string dataStringDate)
        {
            ...
            if (temp.Count > 0)
            {
                ...
                return RedirectToAction("DayDetail", new { dataString = dataStringDate });
            }
            else
            {
