    ...
    public class ManageDriverPayController : Controller
    {
        ...
        public ActionResult Add([Bind(Exclude = "")] DriverPay model, DateTime? date)
        {
            if (ModelState.IsValid)
            {
                ...
                var check = db.DriverPays.Where(x => x.VehicleID == model.VehicleID && x.DriverID == model.DriverID && x.IsRemove != true && x.Content == model.Content
                ...
                if (check)
                {
                    ModelState.AddModelError("", "Đã tồn tại bản ghi trùng tên lái xe, BKS và nội dung trên hệ thống!");
                    return View(model);
                }
                else
                {
        ...
        public ActionResult Edit([Bind(Include = "")] DriverPay model, DateTime? date)
        {
            if (ModelState.IsValid)
            {
                ...
                var check = db.DriverPays.Where(x => x.VehicleID == model.VehicleID && x.DriverID == model.DriverID && x.IsRemove != true && x.ID != model.ID && x.Content == model.Content
                && DbFunctions.TruncateTime(firstDayOfMonth) <= DbFunctions.TruncateTime(x.PayDate) && DbFunctions.TruncateTime(x.PayDate) <= DbFunctions.TruncateTime(lastDayOfMonth)).Any();
