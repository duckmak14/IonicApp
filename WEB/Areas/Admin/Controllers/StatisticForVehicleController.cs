    ...
    public class StatisticForVehicleController : Controller
    {
        ...
        public ActionResult Statistic_Read([DataSourceRequest] DataSourceRequest request, string datetime, bool checkForm)
        {
            if (checkForm)
            {
                ...
                var dataOfRepairVehicle = QueryForRepairVehicle(firstDayOfMonth, lastDayOfMonth);
                ...
                AddIDNotFoundRepairVehicle(dataOfRepairVehicle, manageOil);
                ...
                var dataJoin = (from a in manageOil
                                ...
                                join h in dataOfRepairVehicle on a.VehicleID equals h.VehicleID into group7
                                ...
                                select new
                                {
                                   ...
                                   .Select(B => new StatisticVehicleViewModel()
                                   {
                                       ...
                                       TotalRepairVehicle = B.TotalRepairVehicle
                                   }).ToList();
        ...
        public List<DataJsonRepairVehicle> QueryForRepairVehicle(DateTime firstDayOfMonth, DateTime lastDayOfMonth)
        {
            var dataOfDriverPay = (db.RepairVehicles.Where(x => DbFunctions.TruncateTime(firstDayOfMonth) <= DbFunctions.TruncateTime(x.RepairDate) && DbFunctions.TruncateTime(x.RepairDate) <= DbFunctions.TruncateTime(lastDayOfMonth) && x.IsRemove != true)
                    .Select(k => new { k.Price, k.VehicleID })
                    .GroupBy(x => new { x.VehicleID }, (key, group) => new
                    {
                        VehicleID = (int)key.VehicleID,
                        TotalRepairVehicle = group.Sum(k => k.Price)
                    })).AsEnumerable()
                                       .Select(B => new DataJsonRepairVehicle()
                                       {
                                           VehicleID = B.VehicleID,
                                           TotalRepairVehicle = (Double)B.TotalRepairVehicle
                                       }).ToList();

            return dataOfDriverPay;
        }

        public List<DataJsonRepairVehicle> AddIDNotFoundRepairVehicle(List<DataJsonRepairVehicle> dataJsonRepairVehicles, List<DataJsonManageOil> dataJsonManageOils)
        {
            foreach (var item in dataJsonManageOils)
            {
                var check = false;
                foreach (var i in dataJsonRepairVehicles)
                {
                    if (item.VehicleID == i.VehicleID)
                    {
                        check = true;
                ...
                if (!check)
                {
                    var addData = new DataJsonRepairVehicle();
                    addData.VehicleID = (int)item.VehicleID;
                    dataJsonRepairVehicles.Add(addData);
                }
            }

            return dataJsonRepairVehicles;
        }
        ...
        public List<DataJsonTransportActual> QueryForEnumeration(DateTime firstDayOfMonth, DateTime lastDayOfMonth)
        {
            ...
            var dataOfTransportActual = (db.TransportActuals.Where(x => DbFunctions.TruncateTime(firstDayOfMonth) <= DbFunctions.TruncateTime(x.ActualDate) && DbFunctions.TruncateTime(x.ActualDate) <= DbFunctions.TruncateTime(lastDayOfMonth) && x.IsRemove != true && x.Status == true)
        ...
        {
            List<StatisticVehicleViewModel> listData = new List<StatisticVehicleViewModel>();
            ...
            {
                StatisticVehicleViewModel dataObj = null;
                ...
                dataObj = JsonConvert.DeserializeObject<StatisticVehicleViewModel>(dataObjString);
                dataObj.StatisticVehicleDate = dataObj.StatisticVehicleDate.ToLocalTime();
                listData.Add(dataObj);
            }
            var result = DownloadStatistic(listData);
            var fileStream = new MemoryStream(result);
            return File(fileStream, "application/ms-excel", "Thống_Kê_Chi_Phí_Theo_Xe.xlsx");
        }
        public byte[] DownloadStatistic(List<StatisticVehicleViewModel> models)
        {
            ExcelPackage.LicenseContext = LicenseContext.Commercial;
            var fileinfo = new FileInfo(string.Format(@"{0}\ThongKeTheoXe.xlsx", HostingEnvironment.MapPath("/Uploads")));
            ...
            if (fileinfo.Exists)
            {
                using (var p = new ExcelPackage(fileinfo))
                {
                    ...
                    productWorksheet.Cells[2, 1].Value = models.Select(x => x.StatisticVehicleDate).Min().Month.ToString() + "/" + models.Select(x => x.StatisticVehicleDate).Min().Year.ToString();
                    ...
                    foreach (var listItem in queryPlan)
                    {
                        ...
                        foreach (var item in listItem)
                        {
                            ...
                            productWorksheet.Cells[i, 4].Value = item.TotalParking;
                            productWorksheet.Cells[i, 4].Style.Numberformat.Format = "#,##0";
                            productWorksheet.Cells[i, 5].Value = item.TotalMonthTicket;
                            productWorksheet.Cells[i, 5].Style.Numberformat.Format = "#,##0.00";
                            productWorksheet.Cells[i, 6].Value = item.TotalDateTicket;
                            productWorksheet.Cells[i, 6].Style.Numberformat.Format = "#,##0";
                            productWorksheet.Cells[i, 7].Value = item.TotalRepairVehicle;
                            productWorksheet.Cells[i, 7].Style.Numberformat.Format = "#,##0";
                            productWorksheet.Cells[i, 8].Value = item.TotalDriverPay;
                            productWorksheet.Cells[i, 8].Style.Numberformat.Format = "#,##0";
                            productWorksheet.Cells[i, 9].Value = item.TotalOil;
                            productWorksheet.Cells[i, 9].Style.Numberformat.Format = "#,##0";
                            productWorksheet.Cells[i, 10].Value = item.TotalSalary;
                            productWorksheet.Cells[i, 10].Style.Numberformat.Format = "#,##0";
                            productWorksheet.Cells[i, 11].Value = item.TotalTransportActual;
                            productWorksheet.Cells[i, 11].Style.Numberformat.Format = "#,##0";

                            //productWorksheet.Cells[i, 12].Formula = "=ROUND(D" + i + "+K" + i + ",0)";
                            //productWorksheet.Cells[i, 12].Style.Numberformat.Format = "#,##0";
                            //productWorksheet.Cells[i, 12].Style.Font.Bold = true;
                            i++;
                        }
                    }
