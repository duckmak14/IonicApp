            ...
            if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
            {
                using (var package = new ExcelPackage(file.InputStream))
                {
                    ...
                    for (int rowIterator = 4; rowIterator <= noOfRow; rowIterator++)
                    {
                        ...
                        if (checkDate && repairInfo.VehicleID.HasValue && repairInfo.DriverID.HasValue && repairInfo.CategoryID.HasValue)
                        {
                            var firstDayOfMonth = new DateTime(repairInfo.RepairDate.Year, repairInfo.RepairDate.Month, 1);
                            var lastDayOfMonth = firstDayOfMonth.AddMonths(1).AddDays(-1);

                            var check = repairVehicles.Where(x => x.VehicleID == repairInfo.VehicleID && x.DriverID == repairInfo.DriverID && x.IsRemove != true && x.CategoryID == repairInfo.CategoryID
                            && firstDayOfMonth.Date <= x.RepairDate.Date && x.RepairDate.Date <= lastDayOfMonth.Date).Any();
                            if (check)
                            {
                                isValid = false;
                                AddErrorModels(rowIterator, 1, "Đã tồn tại bản ghi trùng tên lái xe, BKS và danh mục sửa chữa trên hệ thống!");
                            }
                        }
